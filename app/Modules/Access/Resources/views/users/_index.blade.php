<div x-data="{ showFilters: false }">
        ...
        @can('access.users.create')
            <a href="{{ route('access.users.create') }}" wire:navigate
               ...
               style="background: linear-gradient(135deg, #f53003 0%, #c0392b 100%);">
                ...
                {{ __('platform::table.new', ['model' => __('platform::table.users.headers.username')]) }}
            </a>
        @endcan
    </div>
    ...
    >
        ...
        <div class="flex flex-wrap items-end gap-3 p-4">
            <x-ui.ts-table.filter-input
                label="{{ __('platform::table.users.filter_email') }}"
                icon="lucide-search"
                wire:model.live.debounce.400ms="filters.email"
                placeholder="{{ __('platform::table.users.filter_email_placeholder') }}"
            />
    ...
    >
        ...
        @interact('column_status', $row)
            @if ($row->trashed())
                <x-ts-badge text="{{ __('platform::table.users.status_deleted') }}" color="red" />
            @elseif (! $row->is_active)
                <x-ts-badge text="{{ __('platform::table.users.status_inactive') }}" color="yellow" />
            @else
                <x-ts-badge text="{{ __('platform::table.users.status_active') }}" color="green" />
            @endif
        @endinteract
    ...
    @if ($users->total() > 0)
        <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-1.5 text-sm text-content-muted">
                ...
                {{ __('platform::table.results') }} {{ $users->total() }}
            </div>
