<div>
    ...
    <div class="mb-7 text-center">
        <h1 class="text-2xl font-bold mb-1.5" style="color: var(--ui-content);">{{ __('platform::public.welcome_back') }}</h1>
        <p class="text-sm" style="color: var(--ui-content-muted);">{{ __('platform::public.login_subtitle') }}</p>
    </div>
    ...
    <form wire:submit="login" class="flex flex-col gap-4" novalidate>
            ...
            <x-slot:hint>
                <a href="{{ route('password.request') }}" wire:navigate class="text-xs transition-colors duration-200"
                   ...
                   onmouseout="this.style.color='rgba(245,48,3,0.8)';">
                    {{ __('platform::public.forgot_password') }}
                </a>
            </x-slot:hint>
        </x-ui.auth-password>
        ...
        <label class="flex items-center gap-2.5 cursor-pointer select-none" x-data>
            ...
            <span class="text-xs" style="color: var(--ui-content-muted);">{{ __('platform::public.remember_me') }}</span>
        </label>
        ...
        <button type="submit"
            ...
            <span wire:loading.class="opacity-0 -translate-y-2"
                ...
                {{ __('platform::public.login_button') }}
            </span>
    ...
    <div class="flex items-center gap-3 my-6">
        <div class="flex-1 h-px" style="background: var(--auth-divider);"></div>
        <span class="text-xs" style="color: var(--ui-content-subtle);">{{ __('platform::public.no_account') }}</span>
        <div class="flex-1 h-px" style="background: var(--auth-divider);"></div>
    </div>
    ...
    <a href="{{ route('register') }}" wire:navigate
       ...
       onmouseover="this.style.background='var(--auth-link-bg-hover)';this.style.borderColor='var(--auth-link-border-hover)';this.style.color='var(--ui-content)';"
       onmouseout="this.style.background='var(--auth-link-bg)';this.style.borderColor='var(--auth-link-border)';this.style.color='var(--ui-content-muted)';">
        {{ __('platform::public.create_account') }}
    </a>
</div>
