<div>
    ...
    <div class="mb-7 text-center">
        ...
        <h1 class="text-2xl font-bold mb-1.5" style="color: var(--ui-content);">{{ __('platform::public.two_factor_title') }}</h1>
        <p class="text-sm" style="color: var(--ui-content-muted);">
            {{ $showRecovery ? __('platform::public.two_factor_recovery_subtitle') : __('platform::public.two_factor_subtitle') }}
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('two-factor.login') }}" class="flex flex-col gap-4">
        ...
        @if (! $showRecovery)
            <x-ui.auth-input
                name="code"
                wire:model="code"
                label="{{ __('platform::public.two_factor_code') }}"
                placeholder="000000"
        ...
        @else
            <x-ui.auth-input
                ...
                label="{{ __('platform::public.two_factor_recovery_code') }}"
    ...
    <div class="mt-6 text-center">
        <button wire:click="$set('showRecovery', {{ $showRecovery ? 'false' : 'true' }})"
            ...
            {{ $showRecovery ? __('platform::public.use_auth_code') : __('platform::public.use_recovery_code') }}
        </button>
    </div>
</div>
