...
trait HasSoftDeletes
{
    public function confirmDelete(string|int $id): void
    {
        ...
        $this->dialog()
            ->question(
                __('platform::table.dialog.delete.title', ['model' => $model]),
                __('platform::table.dialog.delete.description'),
            )
            ->confirm(__('platform::table.dialog.delete.confirm'), 'softDelete', $id)
            ->cancel(__('platform::table.dialog.cancel'))
            ->send();
    }
    ...
    public function confirmRestore(string|int $id): void
    {
        ...
        $this->dialog()
            ->question(
                __('platform::table.dialog.restore.title', ['model' => $model]),
                __('platform::table.dialog.restore.description'),
            )
            ->confirm(__('platform::table.dialog.restore.confirm'), 'restore', $id)
            ->cancel(__('platform::table.dialog.cancel'))
            ->send();
    }
    ...
    public function restore(string|int $id): void
    {
        ...
        $this->toast()->success(__('platform::app.success'), __('platform::app.restored', ['model' => $this->modelLabel]))->send();
    }
}
