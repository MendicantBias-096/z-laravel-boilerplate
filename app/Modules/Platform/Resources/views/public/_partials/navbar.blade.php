@php
    $links = [
        ['label' => __('platform::public.nav_home'),  'route' => 'home'],
        ['label' => __('platform::public.nav_about'), 'route' => 'platform::public.about'],
    ];
@endphp
        ...
        @auth
            <a href="{{ url('/dashboard') }}" wire:navigate
               ...
               onmouseout="this.style.color='var(--ui-content-muted)';">
                ...
                {{ __('platform::public.nav_dashboard') }}
            </a>
        @else
            @if (Route::has('login'))
                <a href="{{ route('login') }}"
            ...
            @if (Route::has('register'))
                <a href="{{ route('register') }}"
                   ...
                   onmouseout="this.style.boxShadow='none';this.style.transform='translateY(0)';">
                    {{ __('platform::public.nav_register') }}
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
