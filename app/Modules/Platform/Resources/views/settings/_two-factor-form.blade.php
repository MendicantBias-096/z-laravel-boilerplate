    ...
    @if (! $enabled && ! $pending)
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm text-content-muted">{{ __('platform::settings.two_factor_not_enabled') }}</span>
            </div>
            <x-ts-button wire:click="enable" wire:loading.attr="disabled" sm>
                {{ __('platform::settings.two_factor_enable') }}
            </x-ts-button>
        </div>
    @endif
    ...
    @if ($pending)
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm font-medium text-content">{{ __('platform::settings.two_factor_scan') }}</span>
            </div>
            ...
            <p class="text-xs text-content-muted">{{ __('platform::settings.two_factor_qr_instruction') }}</p>

            <div class="flex gap-3">
                <div class="flex-1">
                    <x-ts-input
                        wire:model="confirmationCode"
                        label="{{ __('platform::settings.two_factor_confirm_code') }}"
                        placeholder="000000"
                        inputmode="numeric"
                    />
                </div>
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm font-medium text-content">{{ __('platform::settings.two_factor_active') }}</span>
                </div>
                <x-ts-button wire:click="disable" wire:loading.attr="disabled" sm color="red">
                    {{ __('platform::settings.two_factor_disable') }}
                </x-ts-button>
            </div>

            <div class="flex gap-2">
                <x-ts-button wire:click="showRecoveryCodes" wire:loading.attr="disabled" sm color="secondary">
                    {{ $showingRecovery ? __('platform::settings.recovery_codes_hide') : __('platform::settings.recovery_codes_show') }}
                </x-ts-button>
                <x-ts-button wire:click="regenerateCodes" wire:loading.attr="disabled" sm color="secondary">
                    {{ __('platform::settings.regenerate_codes') }}
                </x-ts-button>
            </div>

            @if ($showingRecovery && count($recoveryCodes))
                <div class="rounded-lg border border-line bg-panel-alt p-4">
                    <p class="mb-3 text-xs text-content-muted">{{ __('platform::settings.recovery_codes_desc') }}</p>
